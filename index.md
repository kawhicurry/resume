...
### 腾讯云 - 官网平台 - SRE 稳定性工程师 `正式` `2024-至今`

**条带化（Set 化）治理**

- 开发元数据管理系统，统一管理 **10+ 服务、300+ 条带**的差异化配置与共性部分
- 提供客户、地域等多个维度的隔离调度，降低故障爆炸半径

**变更流程与流水线建设**

- 建设 **SQL 变更流程**，覆盖审批、灰度、回滚全链路，**覆盖 100% 核心数据库**
- 搭建发布流水线，推进变更的标准化与自动化
- 建设 **AI Agent 驱动的变更流程**，Agent 不直接执行变更，而是发起变更流程，将发现、计划、审批、执行、验证全链路串通

**告警分析与故障定位**

- 基于 **LLM Agent** 构建告警自动分析与故障定位流程，打通 **ES 日志、监控指标**等异构数据源，实现多维度联动分析

### 腾讯音乐娱乐集团 - Q音基础平台 - 业务运维 `实习` `2023/06-2024`
...
### OpenTelemetry SKB — Linux 内核网络可观测性 `2024`
